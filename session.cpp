...
// TODO: No separate reader thread. All reads and writes are done in the same thread.
// Infact the start() function is not required anymore.
void Session::start() {
    if (reader_.joinable()) {
...
// TODO: all these will be part of the epoll wait in the broker server
void Session::run() {
    try {
